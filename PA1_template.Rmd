...
```{r histogram1_2, echo = FALSE, fig.width=4.5}
...
```{r time_series1, echo = FALSE, fig.align='center'}
...
```{r time_series2, echo = FALSE, fig.align='center'}
ggplot(activityDataTS, mapping = aes(interval, avg_steps)) +
        ...
        geom_label(data = activityDataTSMax, nudge_x = 200, 
                   alpha = 0.8, mapping = aes(label = interval))
```
...
```{r time_series3, echo = FALSE, fig.align='center'}
activityData %>%
        group_by(interval) %>%
...
```{r histogram_3_4, echo = FALSE, fig.width=4.5}
...
```{r time_series_4_5, echo = FALSE, fig.width=4.5}
activityData %>%
        group_by(interval) %>%
...
ggplot(activityDataUpdatedWeek, mapping = aes(interval, avg_steps)) +
        ...
        scale_x_discrete(breaks = seq(0, 2400, by = 400))
```
...
```{r histogram1_2_code, eval = FALSE, fig.width=4.5}
ggplot(data = activityDataHistogram, mapping = aes(total_steps)) + 
  geom_histogram(colour = "black", fill = "orangered", 
                 binwidth = 500, boundary = 0) +
  labs(
...
```{r time_series1_code, eval = FALSE, fig.align='center'}
ggplot(data = activityDataTS, mapping = aes(interval, avg_steps)) + 
        geom_line(colour = "blue") +
        labs(
                title ="Average Number of Steps taken per Interval",
...
```{r time_series2_code, eval = FALSE}
ggplot(activityDataTS, mapping = aes(interval, avg_steps)) +
        geom_line(colour = "blue") +
        labs(
                title ="Average Number of Steps taken per Interval",
...
```{r time_series3_code, eval = FALSE, fig.align='center'}
activityData %>%
        group_by(interval) %>%
        summarise(mean_steps = mean(steps, na.rm = TRUE),
                  median_steps = median(steps, na.rm =  TRUE)) %>%
...
```{r histogram_3_4_code, eval = FALSE, fig.width=4.5}
activityDataUpdatedHist <- activityDataUpdated %>%
        group_by(date) %>%
        summarise(total_steps  = sum(steps, na.rm = TRUE))
...
```{r time_series_4_5_code, eval = FALSE, fig.width=4.5}
activityData %>%
        group_by(interval) %>%
        summarise(median_steps = median(steps, na.rm =  TRUE)) %>%
        ggplot() +
...
``` {r panel_plot_code, eval = FALSE, fig.align='center'}
ggplot(activityDataUpdatedWeek, mapping = aes(interval, avg_steps)) +
        geom_line(colour = "blue") +
        facet_grid(week_part ~ .) +
        labs(
